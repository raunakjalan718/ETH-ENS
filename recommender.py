...
class TreatmentRecommender:
    ...
    def load_models_and_data(self):
        try:
            ...
            log.info(f"Loaded model: {type(self.model).__name__}")
            
            # Try filtered encoder first, then regular encoder
            try:
                self.label_encoder = joblib.load(MODELS_DIR / "filtered_label_encoder.pkl")
            except:
                self.label_encoder = joblib.load(PROCESSED_DIR / "label_encoder.pkl")
            ...
            log.info(f"Loaded {len(self.symptom_columns)} symptom columns")
        ...
        for symptom in symptoms:
            ...
            if symptom_normalized in self.symptom_columns:
                ...
                for i, col_symptom in enumerate(self.symptom_columns):
                    if symptom_normalized in col_symptom.lower():
                        feature_vector[i] = 1
                        matched_symptoms.append(col_symptom)
                        break
        
        log.info(f"Matched {len(matched_symptoms)} symptoms")
    ...
    def get_kimi_insights(self, patient_data, disease_predictions):
        try:
            ...
            prompt = f"""
            Medical Analysis for {patient_data['name']} (Age {patient_data['age']}, {patient_data['gender']}):
            
            Symptoms: {symptoms_text}
            Top predicted conditions: {diseases_text}
            
            Provide brief medical insights:
            1. Most likely diagnosis assessment
    ...
    def get_patient_input(self):
        print("\n=== EvoDoc Medical Diagnosis System ===")
        print("Powered by Logistic Regression (85.48% accuracy, 97.52% top-5 accuracy)")
        print()
        ...
        print(f"Examples: fever, cough, headache, nausea, chest_pain, fatigue")
        symptoms_input = input("Symptoms: ").strip()
        symptoms = [s.strip() for s in symptoms_input.split(",") if s.strip()]
        
        allergies_input = input("Allergies (or press Enter if none): ").strip()
    ...
    def display_recommendations(self, patient_data, disease_predictions, ai_insights):
        ...
        print("\n⚠️  DISCLAIMER: This is an AI-generated analysis for educational purposes.")
        print("Always consult with a qualified healthcare professional for medical decisions.")
    ...
    def run_interactive_session(self):
        while True:
            try:
                ...
                continue_input = input("\nAnalyze another patient? (y/n): ").strip().lower()
                if continue_input != 'y':
                    break
